...
if analyze or True:
    ...
    if failed_tickers:
        st.warning(
            f"⚠️ Could not find data for: **{', '.join(failed_tickers)}**. "
            f"They have been excluded. Try adding suffixes like `.NS` for Indian "
            f"stocks or `-USD` for crypto."
        )

    if len(tickers) < 2:
        st.error(
            "Need at least 2 valid tickers to analyze. "
    ...
    if metrics is None:
        st.error(
            ...
            "Please ensure at least 2 tickers have valid data."
        )
        st.stop()
    ...
    with left:
        ...
        ax.set_ylabel('Normalized Price (base=100)', color='white', fontsize=10)
        ...
        metrics_df = pd.DataFrame({
            ...
            'Volatility (%)': ann_vol,
            'Sharpe Ratio': sharpe,
            'Sentiment': [sentiment[t]['label'] for t in tickers],
            'Sentiment Score': [sentiment[t]['avg_sentiment'] for t in tickers],
        })
    ...
    with right:
        ...
        ax2.set_xlim(0, 100)
        ax2.set_xlabel('Risk Score (0-100)', color='white', fontsize=10)
    ...
    worst_sharpe = min(tickers,
                       key=lambda t: (returns[t].mean() * 252 - 0.05) /
                                     (returns[t].std() * np.sqrt(252))
                       if t in returns.columns else 999)
    best_sent = max(tickers, key=lambda t: sentiment[t]['avg_sentiment'])
    worst_sent = min(tickers, key=lambda t: sentiment[t]['avg_sentiment'])
...
- **Best risk-adjusted performer:** {best_sharpe} — highest Sharpe ratio, meaning best return per unit of risk taken.
- **Weakest performer:** {worst_sharpe} — lowest risk-adjusted return. Consider whether it's earning its place in your portfolio.
- **Most positive news sentiment:** {best_sent} — recent headlines are favorable, which may support near-term price stability.
- **Most negative news sentiment:** {worst_sent} — monitor closely for developments that could impact price.
...
""")
    ...
    export_data = {
        'Metric': ['Overall Risk Score', 'Risk Label',
                   'Portfolio Volatility (%)', 'Daily VaR 95% ($)',
                   'Diversification Grade', 'Avg Correlation',
                   'Volatility Score', 'VaR Score',
                   'Correlation Score', 'Sentiment Score'],
